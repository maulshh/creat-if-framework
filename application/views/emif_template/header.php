    <div class="container">
        <div class="navbar-header site-logo">
            <a href="<?= SITE_URL?>" class="navbar-brand">
                <img alt="HMIF" src="<?=base_url('assets').'/'.TEMPLATE?>/images/logo.png">
            </a>
        ...
        <nav class="" role="navigation">
			...
			<?php 	}
				$depth = count($pos);
				if($menu->uri != '#'){?>
				<li title="<?= $menu->title?>" <?php if($pages == $menu->content) echo 'class="active"'?>>
					<a href="<?= base_url($menu->uri);?>">
						<span><!-- <i class="fa fa-home"></i>  --><?= $menu->content?></span>
					</a>
				</li>
			<?php } else {?>
				<li>
					<a href="#" class="dropdown-toggle" data-toggle="dropdown"><?= $menu->content?> <i class="fa fa-angle-down"></i></a>
