...
function Blob(x, y, radius, numPointMasses) {
    ...
    this.drawBody = function (ctx, scaleFactor, body) {
        var i;
        ctx.strokeStyle = "#000000";
        if(body == 'white'){
    ...
    this.draw = function (ctx, scaleFactor, body) {
        var i;
        var up, ori, ang;

        this.drawBody(ctx, scaleFactor, body);
...
function BlobCollective(x, y, startNum, maxNum) {
    ...
    this.draw = function (ctx, scaleFactor, body) {
        ...
        for (i = 0; i < this.blobs.length; i++) {
            ...
            this.blobs[i].draw(ctx, scaleFactor, body);
        }
    }
}
...
function draw(body) {
    ...
    blobColl.draw(ctx, scaleFactor, body);
}

function timeout(body) {
    draw(body);
    update();

    if (stopped == false) {
        setTimeout(function(){timeout(body)}, 30);
    }
}

function init(body, count, max, w, h) {
    if(w)
        width = w;
    if(h)
        height = h;
